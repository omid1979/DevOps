...
resource "libvirt_volume" "base_image" {
  ...
  source = "/var/lib/libvirt/images/rhel9.5-compacted.qcow2" # Path to your base qcow2 image on the KVM host
  format = "qcow2"
}
...
resource "libvirt_domain" "vm" {
  ...
  network_interface {
    ...
    hostname       = "vm-${count.index + 1}"
    addresses      = ["192.168.122.1${count.index + 1}"] # Static IPs: 192.168.100.11 to 192.168.100.15
